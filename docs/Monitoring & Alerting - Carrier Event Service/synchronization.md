...
To handle the tracking data in a specific way,  you can implement the  BAdI /AEB/MA\_EXP\_CONS\_01. The data structure provided in the BAdI is representing a consignment (shipment) with packages and events.

> 📘 Tracking events can be found at different levels
>
> The tracking events can be received for the whole shipment header and also for each handling unit. This depends on the carrier. So make sure you read the data from IM\_CONSIGNMENT- EVENTS and also for IM\_CONSIGNMENT-HANDLINGUNITS ,  HANDLINGUNITS-EVENTS.
